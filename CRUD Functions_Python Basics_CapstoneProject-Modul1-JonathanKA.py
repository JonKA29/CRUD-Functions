#Data yang berisikan nama - nama yang disimpan menggunakan fungsi nested list digabungkan dengan dictionary
ContactList = [
    {'ContactID':1001,
     'Name': 'LeBron James',
     'Occupation': 'Basketball Player',
     'Company Name': 'Los Angeles Lakers',
    ...
    {'ContactID': 1006,
     ...
     'Company Name': 'Mark Cuban Companies',
     'Website': 'https://www.markcubancompanies.com',
     'Email': 'mcuban@gmail.com, mark.cuban@dallasmavs.com'}
]
existingID = int()
i = int()
...
def FindContactID():
    global i #--Menjadikan variable i sebagai global variable.
    global existingID #--Menjadikan variable 'existingID' sebagai global variable.
...
#Fungsi AddContact() memperbolehkan user untuk membuat kontak baru dan menambahkannya ke dalam list.
def AddContact():
    while True:
        try:
...
#Fungsi ModifyContact() adalah untuk mengubah informasi(value) dari sebuah Kontak yang sudah ada didalam list
def ModifyContact():
    while True:
        try:
            ...
            if modContact == 1: #--user memilih opsi 1 - mengedit kontak.
                FindContactID() #--Menjalankan fungsi FindContactID - Untuk mencari ContactID dalam list.
                asktoedit = input('Are you sure you want to update this contact?(yes/no): ').lower()
                if asktoedit == 'yes':
                    #-- Komputer menampilkan menu opsi untuk mengubah value dari kolom yang akan dipilih.
                    editcolumn = int(input('\nModify Contact Menu\n===================\n1. ContactID\n2. Name\n3. Occupation\n4. Company Name\n5. Website\n6. Email\nPlease choose the column name that you want to edit from the contact above:'))
                    if editcolumn == 1: #-- Mengubah ContactID.
                        while True:
                            editContactID = int(input('\nPlease input a NEW value for the existing ContactID: '))
                            for j in range(len(ContactList)):
                                #-- Apabila ContactID yang diinput user sama dengan ContactID dari kontak yang dipilih sebelumnya (alias tidak berubah),...
                                if editContactID == existingID:
                                    while True:
                                        sameID = input('The ContactID that you input for this contact has not been changed. Continue? (yes/no): ') #-- komputer akan print 'ContactID sama/tidak diganti. Lanjutkan?'
                                        if sameID == 'yes':
                                        ...
                                        if sameID == 'no':
                                            ...
                                            print('\nPlease input yes or no')
                                #-- Apabila ContactID yang diinput user sama atau sudah digunakan dalam ContactList, maka komputer akan meminta user untuk menginput ContactID yang baru/belum ada didalam list.
                                elif editContactID == ContactList[j]['ContactID']:
                                    print('\nContactID already exist. Please input a NEW ContactID')
                                    break
                                #-- Apabila ContactID yang diinput baru/belum digunakan dalam list, maka komputer akan memastikan sekali lagi apakah user yakin akan menggantinya.
                                    ...
                                    if updatemakesure == 'no': #-- User input 'no', komputer kembali ke menu ModifyContact()
                                        ModifyContact()
                                    if updatemakesure == 'yes': #-- User input 'yes', komputer akan menjalankan perintah user untuk mengganti ContactID yang lama dengan ContactID yang baru.
                                        ContactList[i]['ContactID'] = editContactID
                                        print(f'''\nContactID = {ContactList[i]['ContactID']}\nName = {ContactList[i]['Name']}\nOccupation = {ContactList[i]['Occupation']}\nCompany Name = {ContactList[i]['Company Name']}\nWebsite = {ContactList[i]['Website']}\nEmail = {ContactList[i]['Email']}''')
                                        print('Data successfully updated')
                                        ContactList.sort(key=SortByID)  #--Mensortir Kontak sesuai ContactID.
                                        ModifyContact()
                    if editcolumn == 2: #-- Memperbolehkan user untuk mengubah value dari 'Nama' per kontak.
                        UpdateColumn('Name')
                    if editcolumn == 3: #-- Memperbolehkan user untuk mengubah value dari 'Pekerjaan' per kontak.
                        UpdateColumn('Occupation')
                    if editcolumn == 4: #-- Memperbolehkan user untuk mengubah value dari 'Nama Perusahaan' per kontak.
                        UpdateColumn('Company Name')
                    if editcolumn == 5: #-- Memperbolehkan user untuk mengubah value dari 'Website' per kontak.
                        UpdateColumn('Website')
                    if editcolumn == 6: #-- Memperbolehkan user untuk mengubah value dari 'Email' per kontak.
                        UpdateColumn('Email')
                    else:
                        print('Please input a correct menu option')
                if asktoedit == 'no':
                    ModifyContact()
                else: #-- Mengingatkan user untuk input yes/no
                    print('Please input yes or no')
                    break
            if modContact == 2:
                MainMenu()
        ...
        except ValueError: #-- Mencegah ValueError
            print("\nPlease input a number/numbers.")

#Fungsi DeleteContact() adalah untuk menghapus kontak yang ada di dalam ContactList
def DeleteContact():
    while True:
        try:
            delContact = int(input('''\n1. Choose a contactID to delete\n2. Back to Main Menu\nWhat would you like to do?\t'''))
            if delContact == 1: #-- Menu 1 - Meminta user input ContactID yang akan di delete.
                contactID = int(input('\nPlease input ContactID you want to delete: '))
                for i in range(len(ContactList)):
                    if contactID == ContactList[i]['ContactID']:
                        #-- Komputer menampilkan detil kontak yang dipilih user
                        print(f'''\nContactID = {ContactList[i]['ContactID']}\nName = {ContactList[i]['Name']}\nOccupation = {ContactList[i]['Occupation']}\nCompany Name = {ContactList[i]['Company Name']}\nWebsite = {ContactList[i]['Website']}\nEmail = {ContactList[i]['Email']}''')
                        deletemenu = input('Contact found. Are you sure you want to delete this contact?(yes/no):  ').lower()
                        while True:
                            if deletemenu == 'yes': #-- Menghapus kontak yang user pilih
                                del ContactList[i]
                                print('\nThis Contact has been deleted.')
                                DeleteContact()
                            if deletemenu == 'no':  #-- Kembali ke DeleteContact()
                                DeleteContact()
                            else: 
                                print('\nPlease input yes or no')
                                deletemenu = input('Contact found. Are you sure you want to delete this contact?(yes/no):  ').lower()
            ...
            elif delContact == 2:   #-- Menu 2 - Kembali ke main menu.
                MainMenu()
            else:
                print('Please input the correct menu.')
        except ValueError:  #-- Mencegah ValueError.
    ...
    ****Here you can view, update, and delete any famous celebrities' contact lists.****
    ====================================================================================
        ...
        if choose_menu == 4:
            DeleteContact()
        if choose_menu == 5:
            print('\nThanks for using Contact Generator. Seeyou!')
            exit()
